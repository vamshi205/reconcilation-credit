...
import { PartyMappingService, PartyNameMapping } from "../services/partyMappingService";
...
export function PartyMappings() {
  const [mappings, setMappings] = useState<PartyNameMapping[]>([]);
  ...
  const loadMappings = async () => {
    try {
      const allMappings = await PartyMappingService.getMappingsByLastUsed();
      setMappings(allMappings);
    } catch (error) {
      console.error('Error loading party mappings:', error);
      setMappings([]);
    }
  };

  const handleStartEdit = (mapping: PartyNameMapping) => {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await PartyMappingService.updateMapping(editingId, {
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Are you sure you want to delete this mapping?")) {
      try {
        await PartyMappingService.deleteMapping(id);
        await loadMappings();
      } catch (error) {
    ...
    <div className="space-y-6">
          ...
          {mappings.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm mt-2">
                Edit party names in the Transactions page to start learning mappings.
              </p>
            </div>
          ) : (
      ...
      <Card>
        ...
        <CardContent className="space-y-2 text-sm text-muted-foreground">
          <p>
            <strong className="text-foreground">Learning:</strong> When you edit a party name in
          ...
          <p>
            ...
            party name that matches a learned mapping, a suggestion badge appears.
          </p>
          <p>
