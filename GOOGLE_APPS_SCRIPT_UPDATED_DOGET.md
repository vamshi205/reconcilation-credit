# Updated doGet Function for Multi-Bank Support

Replace your current `doGet` function with this updated version to support both HDFC and Canara banks:
...
## ⚠️ IMPORTANT: Don't Forget formatDateRows Function!

The `doGet` function calls `formatDateRows()` helper function. Make sure you have this function in your script! Add it AFTER the `doGet` function.

## Key Changes:

1. **Line 343**: Added `sheetName` parameter reading: `const sheetName = e.parameter && e.parameter.sheetName ? e.parameter.sheetName : null;`

2. **Lines 349 & 390**: Use `sheetName` parameter if provided, otherwise default to `'Transactions'` or `'DebitTransactions'` for backward compatibility

3. **Lines 354-366 & 395-407**: If bank-specific sheet doesn't exist, fallback to default sheets

## What This Enables:

✅ **HDFC Credit**: Fetches from `Transactions` sheet (when no `sheetName` provided)  
✅ **Canara Credit**: Fetches from `Canara_CreditTransactions` sheet (when `sheetName=Canara_CreditTransactions`)  
✅ **HDFC Debit**: Fetches from `DebitTransactions` sheet (when no `sheetName` provided)  
✅ **Canara Debit**: Fetches from `Canara_DebitTransactions` sheet (when `sheetName=Canara_DebitTransactions`)  

## Your `doPost` function is already correct!

Your `doPost` function already handles `sheetName` parameter correctly (lines 100, 119, 173), so it will save Canara transactions to the right sheets. You only need to update the `doGet` function above.
